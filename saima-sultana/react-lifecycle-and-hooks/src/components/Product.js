    ...
    return (
        <div>
            {!showProduct ?
                <div>
                    <h2>Product List</h2>
                    {
                        products.map((product, idx) => <div key={idx}>
                            <h3>{idx + 1}.  <b> {product.name}</b> <button className="button" onClick={() => setShowProduct(idx + 1)}> Show Details </button></h3>
                ...
                :
                <ProductDetail productDetails={products[showProduct - 1]}></ProductDetail>
            }
        </div>
    );
};
